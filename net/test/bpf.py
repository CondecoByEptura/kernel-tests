...
def BpfProgLoad(prog_type, instructions):
  ...
  attr = BpfAttrProgLoad((prog_type, len(insn_buff) / BpfInsn._length,
...
def BpfFuncCall(op):
  ...
  dst_src = 0
  ret = BpfInsn((code, dst_src, 0, op))
  return ret.Pack()


def BpfFuncLookupMap():
  return BpfFuncCall(BPF_FUNC_map_lookup_elem)


def BpfFuncUpdateMap():
  return BpfFuncCall(BPF_FUNC_map_update_elem)


def BpfFuncDeleteMap():
  return BpfFuncCall(BPF_FUNC_map_delete_elem)



def BpfFuncGetSocketCookie():
  return BpfFuncCall(BPF_FUNC_get_socket_cookie)



def BpfFuncGetSocketUid():
  return BpfFuncCall(BPF_FUNC_get_socket_uid)
...
def BpfFuncCountPacketInit(map_fd):
  insPackCountStart = [
      ...
      BpfMov64Reg(BPF_REG_2, BPF_REG_7),
      BpfFuncLookupMap(),
...
# Bpf instruction for cgroup bpf filter to accept a packet before exit.
insCgroupAccept = [
    BpfMov64Imm(BPF_REG_0, 1),
    BpfExitInsn()
]

# Bpf instruction for socket bpf filter to accept a packet before exit.
insSkFilterAccept = [
