...
class VtiInterface(object):
  ...
  def SetupXfrm(self, use_null_crypt):
    ...
    self.xfrm.CreateTunnel(xfrm.XFRM_POLICY_IN, None, self.remote, self.local,
                           ...
                           xfrm.ExactMatchMark(self.ikey), None, True)
...
class XfrmVtiTest(xfrm_base.XfrmBaseTest):
  ...
  def _TestVti(self, inner_version, outer_version, direction, func,
    ...
    try:
      ...
      remote_inner = _GetRemoteInnerAddress(inner_version)
      spi = {xfrm.XFRM_POLICY_IN: vti.in_spi, xfrm.XFRM_POLICY_OUT: vti.out_spi}[direction]
  ...
  def ParamTestVtiInput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, xfrm.XFRM_POLICY_IN, self._CheckVtiInput,
                  True)

  def ParamTestVtiOutput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, xfrm.XFRM_POLICY_OUT, self._CheckVtiOutput,
                  True)

  def ParamTestVtiInOutEncrypted(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, xfrm.XFRM_POLICY_IN, self._CheckVtiEncryption,
                  False)

  def ParamTestVtiIcmp(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, xfrm.XFRM_POLICY_OUT, self._CheckVtiIcmp,
                  False)
  ...
  def _CheckVtiRekey(self, vti, inner_version, local_inner, remote_inner):
    ...
    outer_family = net_test.GetAddressFamily(vti.version)
