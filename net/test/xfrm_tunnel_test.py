...
class XfrmTunnelBase(xfrm_base.XfrmBaseTest):
  ...
  def _TestTunnel(self, inner_version, outer_version, func, use_null_crypt):
    ...
    try:
      ...
      #
      # This is required to ensure that encrypted packets (which we currently
      # have no way to easily modify) are not considered replay attacks by the
      # inbound SA. (eg: received 3 packets, seq_num_in = 3, sent only 1,
      # seq_num_out = 1, inbound SA would consider this a replay attack)
      tunnel.TeardownXfrm()
