...
class XfrmVtiTest(xfrm_base.XfrmBaseTest):
  ...
  def _TestVti(self, inner_version, outer_version, spi, func, use_null_crypt):
    ...
    try:
      self._CreateVti(netid, _VTI_NETID, _VTI_IFNAME, outer_version,
                      use_null_crypt)
      rx, tx, next_seq_num = func(netid, _VTI_NETID, _VTI_IFNAME, inner_version,
  ...
  def ParamTestVtiInput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, _TEST_IN_SPI, self._CheckVtiIn,
                  True)

  def ParamTestVtiOutput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, _TEST_OUT_SPI,
                  self._CheckVtiOut, True)
  ...
  def _CheckVtiRekey(self, netid, vti_netid, iface, inner_version,
    ...
    for sel in [xfrm.EmptySelector(AF_INET), xfrm.EmptySelector(AF_INET6)]:
      ...
      tmpl = xfrm_base.UserTemplate(outer_family, 0, 0,
                                    (remote_outer, local_outer))
    ...
    tx, rx, new_seq_num_in = self._CheckVtiIn(
        netid, vti_netid, iface, inner_version, outer_version, local_inner,
        remote_inner, local_outer, remote_outer, tx, rx, new_in_spi,
        new_seq_num_in)
    tx, rx, new_seq_num_out = self._CheckVtiOut(
        netid, vti_netid, iface, inner_version, outer_version, local_inner,
    ...
    # Update inbound policy to enforce new SPI; disable all old SPIs
    for sel in [xfrm.EmptySelector(AF_INET), xfrm.EmptySelector(AF_INET6)]:
      policy = xfrm_base.UserPolicy(xfrm.XFRM_POLICY_IN, sel)
      tmpl = xfrm_base.UserTemplate(outer_family, new_in_spi, 0,
                                    (remote_outer, local_outer))
      self.xfrm.UpdatePolicyInfo(policy, tmpl, xfrm.ExactMatchMark(_TEST_IKEY))

    # Even if the SA is recreated, expect receive to fail due to policy update.
    self.xfrm.AddSaInfo(
        remote_outer, local_outer,
        _TEST_IN_SPI, xfrm.XFRM_MODE_TUNNEL, 0,
        xfrm_base._ALGO_CRYPT_NULL,
        xfrm_base._ALGO_AUTH_NULL,
        None,
        None,
  ...
  def ParamTestVtiRekey(self, inner_version, outer_version):
    ...
    try:
      self._CreateVti(netid, _VTI_NETID, _VTI_IFNAME, outer_version, True)
