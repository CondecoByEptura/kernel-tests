...
class XfrmTunnelTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  # Combine an IPsec Tunnel with a VTI to do something useful
  def testVtiOutput(self):
    ...
    try:
      ...
      net_test.SetInterfaceUp("test_vti")
      self.createXfrmTunnel(
          direction=xfrm.XFRM_POLICY_OUT,
          addr_family=AF_INET,
          src_addr="0.0.0.0",
      ...
      #print "ip tunnel"
      #subprocess.call("ip tunnel".split())
      #print "ip link show"
      ...
      for version in [4]:
        ...
        # route locally originating packets to the VTI route table
        self.iproute.IifRule(version, True, "lo", _VTI_TABLE_ID, 400)
        # add a rule to the VTI route table to send packets to the VTI
        self.iproute.AddRoute(version, _VTI_TABLE_ID, "default", 0, router,
                              ifindex)
