...
_TEST_OKEY = _TEST_OUT_SPI
_TEST_IKEY = _TEST_IN_SPI
_TEST_OUT_MARK = _TEST_OKEY
_TEST_IN_MARK = _TEST_IKEY
...
class XfrmTunnelTest(xfrm_base.XfrmBaseTest):
  ...
  def testAddTunnel(self):
    ...
    for inner_version in [4, 6]:
      for outer_version in [4, 6]:
        ...
        try:
          local_inner = xfrm_base.GetWildcardAddress(inner_version)
          ...
          self._CreateXfrmTunnel(
              ...
              inner_family=xfrm_base.GetAddressFamily(inner_version),
              src_addr=local_inner,
              dst_addr=remote_inner,
              outer_family=xfrm_base.GetAddressFamily(outer_version),
          ...
          s = socket(xfrm_base.GetAddressFamily(inner_version), SOCK_DGRAM, 0)
  ...
  def _CheckVtiOutput(self, inner_version, outer_version):
    ...
    try:
      inner_addr = xfrm_base.GetWildcardAddress(inner_version)
      self._CreateXfrmTunnel(
          direction=xfrm.XFRM_POLICY_OUT,
          inner_family=xfrm_base.GetAddressFamily(inner_version),
          src_addr=inner_addr,
      ...
      if (packets):
        packets[0].show()
      self.assertEquals(0, len(packets), "Unexpected packets received!")
