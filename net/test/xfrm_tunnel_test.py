...
class XfrmVtiTest(xfrm_base.XfrmBaseTest):
  ...
  def _CheckVtiInput(self, vti, inner_version, local_inner, remote_inner, spi,
                     seq_num, expect_fail=False):
    read_sock, local_port = _CreateReceiveSock(inner_version)
  ...
  def _TestVti(self, inner_version, outer_version, direction, func,
               use_null_crypt):
  ...
  def ParamTestVtiInput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, "IN", self._CheckVtiInput,
                  True)

  def ParamTestVtiOutput(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, "OUT", self._CheckVtiOutput,
                  True)

  def ParamTestVtiInOutEncrypted(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, "IN", self._CheckVtiEncryption,
                  False)

  def ParamTestVtiIcmp(self, inner_version, outer_version):
    self._TestVti(inner_version, outer_version, "OUT", self._CheckVtiIcmp,
                  False)
  ...
  def _CheckVtiRekey(self, vti, inner_version, local_inner, remote_inner):
    ...
    # Delete old SAs
    self.xfrm.DeleteSaInfo(vti.local, old_in_spi, IPPROTO_ESP,
                           xfrm.ExactMatchMark(vti.ikey))
    self.xfrm.DeleteSaInfo(vti.remote, old_out_spi, IPPROTO_ESP,
                           xfrm.ExactMatchMark(vti.okey))
