...
class XfrmTunnelTest(xfrm_base.XfrmBaseTest):
  ...
  def _GetLocalInnerAddress(self, version):
    return "10.16.5.15" if version is 4 else "2001:db8:1::1"

  def _GetRemoteInnerAddress(self, version):
    return "10.16.5.20" if version is 4 else "2001:db8:2::1"

  def _GetRemoteOuterAddress(self, version):
    return self.GetRemoteAddress(version)

  def _GetWildcardAddress(self, version):
    return "0.0.0.0" if version is 4 else "::"
      ...
      for outer_version in [4, 6]:
        ...
        try:
          ...
          self._CreateXfrmTunnel(
              ...
              inner_family=(AF_INET if inner_version is 4 else AF_INET6),
              src_addr=local_inner,
              dst_addr=remote_inner,
              outer_family=(AF_INET if outer_version is 4 else AF_INET6),
          ...
          s = socket((AF_INET
                      if inner_version is 4 else AF_INET6), SOCK_DGRAM, 0)
