...
class XfrmTunnelTest(xfrm_base.XfrmBaseTest):
  ...
  def setUp(self):
    ...
    self.assertNotIn(_VTI_NETID, self.NETIDS, "VTI Netid In use")
    self.iproute = iproute.IPRoute()
    try:
        self.iproute.DeleteLink(_VTI_IFNAME)
    except IOError as possible:
        pass
  ...
  def _DeleteXfrmTunnel(self, daddr, spi, proto):
    pass
    ...
    for version in [4, 6]:
      ...
      if is_add:
        ...
        self.iproute.AddAddress(
            ...
            net_test.AddressLengthBits(version),
            ifindex)
