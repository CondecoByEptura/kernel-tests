...
_LOCAL_INNER_ADDR1 = {4: "10.16.5.15", 6: "2600:0001::1"}
_REMOTE_INNER_ADDR1 = {4: "10.16.5.20", 6: "2601:0000::1"}

_TUNNEL_REMOTE_ADDR = {4: "8.8.8.8", 6: "2600::8080:4040"}

_INADDR_ANY = {4: "0.0.0.0", 6: "::"}
  ...
  def testAddVti(self):
    for ver in [4]:
      ...
      self.tunnel.CreateVti(
          ...
          local_addr=_REMOTE_INNER_ADDR1[ver],
  ...
  def _SetupVtiNetwork(self, ifname, is_add):
    for version in [4, 6]:
      ...
      self.iproute.UidRangeRule(version, is_add, start, end, table,
                                self.PRIORITY_UID - 1)
      self.iproute.OifRule(version, is_add, ifname, table,
                           self.PRIORITY_OIF - 1)
      self.iproute.FwmarkRule(version, is_add, _VTI_NETID, table,
                              self.PRIORITY_FWMARK - 1)
      if is_add:
        if version == 4:
    ...
    for i_ver in [4, 6]:
      for o_ver in [4]:
        ...
        self.iproute.AddAddress(_LOCAL_INNER_ADDR1[i_ver],
                                ...
                                self.tunnel.GetIfIndex(_VTI_IFNAME))
