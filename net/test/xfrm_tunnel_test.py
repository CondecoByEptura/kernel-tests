    ...
    for version in [4, 6]:
      ...
      if is_add:
        ...
        self.iproute.AddAddress(
            ...
            net_test.AddressLengthBits(version), ifindex, iproute.IFA_F_NODAD
            if version is 6 else 0)
  ...
  def _CheckVtiOutput(self, inner_version, outer_version):
    ...
    try:
      ...
      self.iproute.DelAddress(
          local,
          net_test.AddressLengthBits(inner_version), ifindex)
      # Give the address some time to get added. Even though we specify
      # IFA_F_NODAD, there's still a race, because when inet6_addr_add returns,
      # the input route has not yet been added to the local table. The route is
      # added in addrconf_dad_begin with a delayed timer of 0, but if the system
      # is under load, we could win the race against that timer.
      # TODO: instead of doing this, listen for RTM_NEWADDR in AddAddress.
      time.sleep(0.1)
      try:
