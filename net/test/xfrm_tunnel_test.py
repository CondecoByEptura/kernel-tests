...
class XfrmTunnelTest(xfrm_base.XfrmBaseTest):
  ...
  def _CreateXfrmTunnel(self, direction, inner_family, src_addr, dst_addr,
    ...
    self.xfrm.AddMinimalSaInfo(
        ...
        mark, xfrm_base._MARK_MASK_ALL if mark is not None else None)
    ...
    tmpl = xfrm.XfrmUserTmpl(
        ...
        share=xfrm.XFRM_SHARE_ANY,
        optional=0,  # require
        aalgos=xfrm_base._ALL_ALGORITHMS,  # auth algos
        ealgos=xfrm_base._ALL_ALGORITHMS,  # encryption algos
        calgos=xfrm_base._ALL_ALGORITHMS)  # compression algos

    self.xfrm.AddPolicyInfo(policy, tmpl,
                            xfrm.XfrmMark((mark, xfrm_base._MARK_MASK_ALL))
                            if mark else None)
