...
class XfrmTunnelTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testAddVti(self):
    ...
    self.assertNotEquals(-1, if_index)
  ...
  def testVtiOutput(self):
    ...
    self.tunnel.CreateVti(
        ...
        i_key=_DEFAULT_IKEY_1,
        o_key=_DEFAULT_OKEY_1)
    self.AddInterface(_VTI_NETID, _VTI_IFNAME)
    net_test.SetInterfaceUp(_VTI_IFNAME)

    self._RunSetupCommands(_VTI_NETID, has_router=False)
    ...
    try:
      ...
      self._RunTeardownCommands(_VTI_NETID, has_router=False)
