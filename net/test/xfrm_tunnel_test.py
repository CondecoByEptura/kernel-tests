...
def InjectParameterizedTests(cls):
  ...
  # Test all inner and outer version combinations.
  ...
  def testAddVti(self):
    ...
    for version in [4, 6]:
      ...
      self.iproute.CreateVirtualTunnelInterface(
          ...
          o_key=_TEST_OKEY,
          i_key=_TEST_IKEY)
      self._VerifyVtiInfoData(self.iproute.GetIfinfoData(_TEST_XFRM_IFNAME),
                             version, local_addr,
...
class XfrmTunnelBase(xfrm_base.XfrmBaseTest):
  ...
  def setUpClass(cls):
    ...
    for i, underlying_netid in enumerate(cls.tuns):
      for version in 4, 6:
        netid = _BASE_TUNNEL_NETID[version] + i
        iface = "ipsec%s" % netid
  ...
  def randomTunnel(self, outer_version):
    version_dict = self.__class__.tunnelsV4 if outer_version == 4 else self.__class__.tunnelsV6
  ...
  def _CheckTunnelInput(self, tunnel, inner_version, local_inner, remote_inner,
    ...
    input_pkt = _GetNullAuthCryptTunnelModePkt(
        ...
        local_inner, tunnel.local, local_port, spi, seq_num)
  ...
  def _CheckTunnelOutput(self, tunnel, inner_version, local_inner, remote_inner,
                      spi, seq_num):
    ...
    pkt = self._ExpectEspPacketOn(tunnel.underlying_netid, tunnel.out_spi,
                                  seq_num, None, tunnel.local, tunnel.remote)
    ...
    expected = _GetNullAuthCryptTunnelModePkt(
        ...
        tunnel.remote, _TEST_REMOTE_PORT, tunnel.out_spi, seq_num,
        ip_hdr_options)
  ...
  def _TestTunnel(self, inner_version, outer_version, direction, func,
                  use_null_crypt):
    ...
    try:
      ...
      remote_inner = _GetRemoteInnerAddress(inner_version)
      spi = {
          xfrm.XFRM_POLICY_IN: tunnel.in_spi,
          xfrm.XFRM_POLICY_OUT: tunnel.out_spi
      }[direction]

      next_seq_num = func(tunnel, inner_version, local_inner, remote_inner, spi,
                          1)
      next_seq_num = func(tunnel, inner_version, local_inner, remote_inner, spi,
                          next_seq_num)
    finally:
  ...
  def _CheckTunnelRekey(self, tunnel, inner_version, local_inner, remote_inner):
    ...
    old_out_spi = tunnel.out_spi
    old_in_spi = tunnel.in_spi

    # Check to make sure that both directions work before rekey
    seq_num_in = self._CheckTunnelInput(tunnel, inner_version, local_inner,
                                        remote_inner, old_in_spi, seq_num_in)
    seq_num_out = self._CheckTunnelOutput(tunnel, inner_version, local_inner,
                                          remote_inner, old_out_spi,
                                          seq_num_out)
    ...
    # Perform Rekey
    tunnel.Rekey(outer_family, new_out_spi, new_in_spi)

    # Update Interface object
    tunnel.out_spi = new_out_spi
    tunnel.in_spi = new_in_spi
    ...
    new_seq_num_in = self._CheckTunnelInput(tunnel, inner_version, local_inner,
                                            remote_inner, new_in_spi,
                                            new_seq_num_in)
    new_seq_num_out = self._CheckTunnelOutput(tunnel, inner_version,
                                              local_inner, remote_inner,
                                              new_out_spi, new_seq_num_out)

    # Delete old SAs
    tunnel.DeleteOldSaInfo(outer_family, old_in_spi, old_out_spi)
...
class XfrmVtiTest(XfrmTunnelBase):
  ...
  def ParamTestVtiInput(self, inner_version, outer_version):
    self._TestTunnel(inner_version, outer_version, xfrm.XFRM_POLICY_IN,
  ...
  def ParamTestVtiIcmp(self, inner_version, outer_version):
    self._TestTunnel(inner_version, outer_version, xfrm.XFRM_POLICY_OUT,
