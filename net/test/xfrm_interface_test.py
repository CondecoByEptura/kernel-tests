...
class XfrmInterfaceTest(xfrm_base.XfrmLazyTest):
  ...
  def _CreateXfrmTunnel(self,
    ...
    self.xfrm.AddSaInfo(
        ...
        None,
        None,
        output_mark, xfrm_if_id=xfrm_if_id)

    xfrm_if_id = struct.pack("=I", xfrm_if_id)
    policy = xfrm.UserPolicy(direction, selector)
    tmpl = xfrm.UserTemplate(outer_family, spi, 0, (tsrc_addr, tdst_addr))

    self.xfrm.AddPolicyInfo(policy, tmpl, None, xfrm_if_id=xfrm_if_id)


  def CreateXfrmInterface(self, dev_name, netid, xfrm_if_id):
    """Create an XFRM interface."""
    ifindex = self.ifindices[netid]

    ifdata = self.iproute._NlAttrU32(IFLA_XFRM_LINK, ifindex)
    ifdata += self.iproute._NlAttrU32(IFLA_XFRM_IF_ID, xfrm_if_id)
  ...
  def testAddXfrmInterface(self):
    ...
    self.CreateXfrmInterface(_XFRM_IFNAME, netid, _TEST_XFRM_IF_ID)
  ...
  def _SetupXfrmNetwork(self, ifname, is_add):
    ...
    for version in [4, 6]:
      ...
      self.iproute.FwmarkRule(version, is_add, _XFRM_NETID, 0xffffffff, table,
...
#      subprocess.call("ip rule".split())
      dst = (self._GetRemoteInnerAddress(inner_version), port)
      write_sock.sendto(net_test.UDP_PAYLOAD, dst)
