...
class XfrmTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testAddSa(self):
    encryption = xfrm.XfrmAlgo(("cbc(aes)", 256))
    auth_trunc = xfrm.XfrmAlgoAuth(("hmac(sha1)", 128, 96))
    self.xfrm.AddMinimalSaInfo("::", TEST_ADDR1, htonl(0x1234), IPPROTO_ESP,
    ...
    try:
      ...
      self.xfrm.DeleteSaInfo(TEST_ADDR1, htonl(0x1234), IPPROTO_ESP)
...
#    xfrmid = xfrm.XfrmId((xfrm.RawAddress(daddr), htonl(0x1234), IPPROTO_ESP))
    ...
    xfrmid = xfrm.XfrmId((XFRM_ADDR_ANY, htonl(0x1234), IPPROTO_ESP))
    ...
    s.sendto(net_test.UDP_PAYLOAD, (TEST_ADDR1, 53))
    self.expectIPv6EspPacketOn(netid, 0x1234, 1, 84)
  ...
  def testUdpEncapWithSocketPolicy(self):
    ...
    encaptmpl = xfrm.XfrmEncapTmpl((htons(xfrm.UDP_ENCAP_ESPINUDP), encap_port,
                                    4500, 16 * "\x00"))
    encryption = xfrm.XfrmAlgo(("cbc(aes)", 256))
    auth_trunc = xfrm.XfrmAlgoAuth(("hmac(sha1)", 128, 96))
    self.xfrm.AddMinimalSaInfo(myaddr, remoteaddr, out_spi, IPPROTO_ESP,
                               ...
                               encryption, ENCRYPTION_KEY,
                               auth_trunc, AUTH_TRUNC_KEY, encaptmpl)
