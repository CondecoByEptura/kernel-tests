...
class XfrmFunctionalTest(xfrm_base.XfrmBaseTest):
  ...
  def _TestSocketPolicy(self, version):
    ...
    self.xfrm.AddMinimalSaInfo(net_test.GetWildcardAddress(xfrm_version),
    ...
    self.xfrm.DeleteSaInfo(self.GetRemoteAddress(xfrm_version),
                           htonl(TEST_SPI), IPPROTO_ESP)
    self.assertRaisesErrno(
        EAGAIN,
        s.sendto, net_test.UDP_PAYLOAD, (remoteaddr, 53))
    ...
    xfrm_base.SetSocketPolicyOption(s, family, None)
    s.sendto(net_test.UDP_PAYLOAD, (remoteaddr, 53))
