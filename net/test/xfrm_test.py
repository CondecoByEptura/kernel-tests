...
class XfrmFunctionalTest(xfrm_base.XfrmBaseTest):
  ...
  def _VerifyUdpEncapSocket(self, netid, remoteaddr, myaddr, encap_port, sock,
    ...
    trunc_len =
        xfrm_base._AUTH_NULL_TRUNC_LEN
        if null_auth else xfrm_base._AUTH_SHA1_TRUNC_LEN
    expected_len = xfrm_base.GetEspPacketLength(
        xfrm.XFRM_MODE_TRANSPORT, 4, True, net_test.UDP_PAYLOAD,
        xfrm_base._CRYPT_CBC_BLK_SIZE, xfrm_base._CRYPT_CBC_BLK_SIZE, trunc_len)
