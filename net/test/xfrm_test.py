...
class XfrmTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testAllocRangeSpiUnavailable(self):
    ...
    spis = set()
    with self.assertRaisesErrno(ENOENT):
      # Due to the way kernel picks random SPIs, this has a high probability of
      # returning ENOENT before reaching the size of the range.
  ...
  def testAllocSpiSideEffect(self):
    ...
    # TODO: What is the strategy for testing broken behaviors? (see also:
    # testInboundSaCollision)
