...
class XfrmFunctionalTest(xfrm_base.XfrmLazyTest):
  ...
  def _CheckUDPEncapRecv(self, version, mode):
    ...
    if mode == xfrm.XFRM_MODE_TRANSPORT:
      ...
      data = 19 * b"a"
      datalen = len(data)
      data += xfrm_base.GetEspTrailer(len(data), 1, IPPROTO_UDP)
  ...
  @unittest.skipUnless(net_test.LINUX_VERSION >= (5, 10, 108),
                       reason="Unsupported pre-5.10, broken up to 5.10.107")
