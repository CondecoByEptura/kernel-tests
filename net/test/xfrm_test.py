...
class XfrmFunctionalTest(xfrm_base.XfrmBaseTest):
  ...
  def _CheckNullEncryptionTransportMode(self, version):
    ...
    # Output
    self.xfrm.AddSaInfo(
        net_test.GetWildcardAddress(version), remote_addr, 0xABCD, xfrm.XFRM_MODE_TRANSPORT, 123,
        xfrm_base._ALGO_CRYPT_NULL, xfrm_base._ALGO_AUTH_NULL,
        None, None, None, None)
    # Input
    self.xfrm.AddSaInfo(
        remote_addr, net_test.GetWildcardAddress(version), 0x9876, xfrm.XFRM_MODE_TRANSPORT, 456,
        xfrm_base._ALGO_CRYPT_NULL, xfrm_base._ALGO_AUTH_NULL,
        None, None, None, None)
