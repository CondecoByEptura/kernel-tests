...
class XfrmExpiryTest(xfrm_base.XfrmLazyTest):
  ...
  def setupSocketAndSas(self, netid, version, remote_addr, lifetime_cfg):
    ...
    self.xfrm.AddSaInfo(net_test.GetWildcardAddress(xfrm_version),
                        ...
                        1234, xfrm_base._ALGO_CBC_AES_256, xfrm_base._ALGO_AUTH_NULL, None,
                        None, None, None, lifetime_cfg=lifetime_cfg)

    return s

  def GetExpectedPacketSize(self, version):
    return xfrm_base.GetEspPacketLength(xfrm.XFRM_MODE_TRANSPORT,
                                        version, False,
  ...
  def VerifyPacketCountExpiry(self, version):
    ...
    lifetime_cfg = xfrm.XfrmLifetimeCfg((xfrm._INF, xfrm._INF, 1, 2, 0, 0, 0, 0))
    sock = self.setupSocketAndSas(netid, version, remote_addr, lifetime_cfg)

    # Build and register a listener for expiry events
    xfrm_listener = xfrm.Xfrm(groups=self.getListenerGroup(xfrm.XFRM_NL_GRP_EXPIRE))
    ...
    self.assertRaisesErrno(EINVAL, sock.sendto, net_test.UDP_PAYLOAD, (remote_addr, 53))
  ...
  def VerifyByteCountExpiry(self, version):
    ...
    lifetime_cfg = xfrm.XfrmLifetimeCfg((len(net_test.UDP_PAYLOAD) - 1, len(net_test.UDP_PAYLOAD) * 2 - 1, xfrm._INF, xfrm._INF, 0, 0, 0, 0))
    sock = self.setupSocketAndSas(netid, version, remote_addr, lifetime_cfg)
