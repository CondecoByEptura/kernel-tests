...
class TunTwister(object):
  ...
  def _Flush(self):
    ...
    while p.poll(TunTwister._POLL_FAST_TIMEOUT):
      self._ProcessPacket()
...
class TapTwister(TunTwister):
  ...
  def __init__(self, fd=None, validator=None):
    ...
    TapTwister works just like TunTwister, but handles ethernet headers.
