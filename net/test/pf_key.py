...
# Maps extension header names to their data structures.
def ExtensionToType(exttype):
  if exttype == SADB_EXT_SA:
    return SadbSa
  if exttype == SADB_X_EXT_SA2:
    return SadbXSa2
  if SADB_X_EXT_NAT_T_TYPE:
    return SadbXNatTType
  if name in [SADB_X_EXT_NAT_T_SPORT, SADB_X_EXT_NAT_T_DPORT]:
    return SadbXNatTPort
  return None
...
class PfKey(object):
  ...
  def ParseExtensions(data):
    ...
    extensions = []
    while data:
      hdr, data = cstruct.Read(data, SadbExt)
      datalen = PfKey.ExtensionsLength(hdr, SadbExt)
      extdata, data = data[:datalen], data[datalen:]
      attrs = ""
      struct_type = ExtensionToType(hdr.exttype)
      if struct_type:
        ext, attrs = cstruct.Read(extdata, struct_type)
      else:
        ext, attrs = extdata, ""
      extensions.append((hdr.exttype, ext, attrs))
