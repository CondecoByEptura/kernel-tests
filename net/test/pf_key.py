...
class PfKey(object):
  ...
  def MakeSadbMsg(self, msgtype, satype):
    # errno is 0. seq, pid and len are filled in by Send().
    return SadbMsg((PF_KEY_V2, msgtype, 0, satype, 0, 0, 0, 0))

  def MakeSadbExtAddr(self, exttype, addr):
    family = struct.unpack("=H", addr[:2])[0]
    prefixlen = {AF_INET: 32, AF_INET6: 128}[family]
    padbytes = (len(SadbExt) + len(SadbAddress) + len(addr)) % 8
    addr = addr + "\x00" * padbytes
    return (exttype, SadbAddress((0, prefixlen)), addr)
  ...
  def AddSa(self, src, dst, spi, satype, mode, reqid, encryption,
    ...
    self.Send(msg, self.PackPfKeyExtensions(extlist))
    reply = SadbMsg(self.sock.recv(4096))
    if reply.errno != 0:
  ...
  @staticmethod
  def PrintPfKeyExtensions(data):
    while data:
      ext, data = cstruct.Read(data, SadbExt)
      datalen = PfKey.ExtensionsLength(ext, SadbExt)
      extdata, data = data[:datalen], data[datalen:]
      attrs = ""
      exttype = _GetMultiConstantName(ext.exttype, ["SADB_EXT",
                                                    "SADB_X_EXT"])
      if ext.exttype == SADB_EXT_SA:
        sa, attrs = cstruct.Read(extdata, SadbSa)
        print " ", exttype, PfKey.DecodeSadbSa(sa), attrs.encode("hex")
      elif ext.exttype in [SADB_EXT_LIFETIME_CURRENT,
                           SADB_EXT_LIFETIME_HARD,
                           SADB_EXT_LIFETIME_SOFT]:
        lft, attrs = cstruct.Read(extdata, SadbLifetime)
        print " ", exttype, lft, attrs.encode("hex")
      elif ext.exttype in [SADB_EXT_ADDRESS_SRC, SADB_EXT_ADDRESS_DST,
                           SADB_EXT_ADDRESS_PROXY]:
        addr, attrs = cstruct.Read(extdata, SadbAddress)
        print " ", exttype, addr, attrs.encode("hex")
      elif ext.exttype in [SADB_EXT_KEY_AUTH, SADB_EXT_KEY_ENCRYPT]:
        key, attrs = cstruct.Read(extdata, SadbKey)
        print " ", exttype, key, attrs.encode("hex")
      elif ext.exttype == SADB_X_EXT_SA2:
        sa2, attrs = cstruct.Read(extdata, SadbXSa2)
        print " ", exttype, sa2, attrs.encode("hex")
      elif ext.exttype == SADB_X_EXT_NAT_T_TYPE:
        natttype, attrs = cstruct.Read(extdata, SadbXNatTType)
        print " ", exttype, natttype, attrs.encode("hex")
      elif ext.exttype in [SADB_X_EXT_NAT_T_SPORT, SADB_X_EXT_NAT_T_DPORT]:
        nattport, attrs = cstruct.Read(extdata, SadbXNatTPort)
        print " ", exttype, nattport, attrs.encode("hex")
      else:
        print "  %d %s %s" % (datalen, exttype, extdata.encode("hex"))

  def DumpPfKeySaInfo(self):
    msglen = len(SadbMsg) / 8
    msg = SadbMsg((PF_KEY_V2, SADB_DUMP, 0, 0, msglen, 0, 0, os.getpid()))
    self.Send(msg, "")
    ...
    while received:
      ...
      self.PrintPfKeyExtensions(extensions)
      if msg.seq == 0:
        break
      received = self.sock.recv(4096)
