...
class IPRoute(netlink.NetlinkSocket):
  ...
  def CreateVti(self, dev_name, local_addr, remote_addr, i_key=None,
    ...
    """
    ifinfo = IfInfoMsg(family=socket.AF_UNSPEC).Pack()
    ...
    return self._SendNlRequest(iproute.RTM_NEWLINK, ifinfo, flags)

  def DeleteTunnel(self, dev_name):
    ifinfo = IfInfoMsg(family=socket.AF_UNSPEC).Pack()
    ifinfo += self._NlAttrStr(IFLA_IFNAME, dev_name)
    flags = netlink.NLM_F_REQUEST | netlink.NLM_F_ACK
    return self._SendNlRequest(iproute.RTM_DELLINK, ifinfo, flags)

  def GetIfIndex(self, dev_name):
    ifinfo = IfInfoMsg(family=socket.AF_UNSPEC).Pack()
    ifinfo += self._NlAttrStr(IFLA_IFNAME, dev_name)
    self._SendNlRequest(iproute.RTM_GETLINK, ifinfo, netlink.NLM_F_REQUEST)
    hdr, data = cstruct.Read(self._Recv(), netlink.NLMsgHdr)
    if hdr.type == iproute.RTM_NEWLINK:
