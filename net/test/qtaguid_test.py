...
class QtaguidTest(tcp_test.TcpBaseTest):
  ...
  def CheckUidGidCombination(self, version, invert_gid, invert_uid):
    ...
    myGid = os.getgid()
    if invert_gid:
      self.AddIptablesInvertedRule(version, True, myGid)
    else:
      self.AddIptablesRule(version, True, 12345)
    if invert_uid:
      self.AddIptablesInvertedRule(version, False, myUid)
    else:
      self.AddIptablesRule(version, False, 12345)
    for netid in self.NETIDS:
      try:
        self.TcpFullStateCheck(version, netid)
      except AssertionError as e:
        if e.message == "expecting TCP RST: : received no packets" and invert_gid:
          pass
        else:
          raise
    if invert_gid:
      self.DelIptablesInvertedRule(version, True, myGid)
    else:
      self.DelIptablesRule(version, True, 12345)
    if invert_uid:
      self.AddIptablesInvertedRule(version, False, myUid)
    else:
      self.DelIptablesRule(version, False, 12345)
  ...
  def testUidGidCombineMatch(self):
    self.CheckUidGidCombination(4, True, True)
    self.CheckUidGidCombination(4, True, False)
    self.CheckUidGidCombination(4, False, True)
    self.CheckUidGidCombination(4, False, False)
    self.CheckUidGidCombination(6, True, True)
    self.CheckUidGidCombination(6, True, False)
    self.CheckUidGidCombination(6, False, True)
    self.CheckUidGidCombination(6, False, False)
