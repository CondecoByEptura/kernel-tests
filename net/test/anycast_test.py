...
class AnycastTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testAnycastNetdeviceUnregister(self):
    ...
    # For kernels with MPTCP ported, closing tun interface need more than 0.5 sec.
    # DAD procedure within MPTCP fullmesh module takes more time, because duplicate
    # address-timer takes a refcount on the IPv6-address, preventing it from getting closed.
    thread.join(3)
