...
class MulticastRoutingTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def setUpClass(cls):
    ...
    cls.netid1 = cls.NETIDS[0]
    cls.netid2 = cls.NETIDS[1]
    cls.netid3 = cls.NETIDS[2]
    cls.virtualIndexes = {}
    cls.virtualIndexes[cls.netid1] = 0
    cls.virtualIndexes[cls.netid2] = 1
    cls.virtualIndexes[cls.netid3] = 2
  ...
  def setUp(self):
    ...
    # add the 3 interfaces as multicast interfaces
    self.AddMulticastInterface(self.netid1)
    self.AddMulticastInterface(self.netid2)
    self.AddMulticastInterface(self.netid3)

  def tearDown(self):
    super(MulticastRoutingTest, self).tearDown()

    # remove the 3 interfaces as multicast interfaces
    self.RemoveMulticastInterface(self.netid1)
    self.RemoveMulticastInterface(self.netid2)
    self.RemoveMulticastInterface(self.netid3)
  ...
  def testEnableSrcToGroupForwarding(self):
    # enable forwarding (S, G) from if1 to if2:
    self.EnableSourceToGroupRouting(self.netid1, [self.netid2])

    self.CheckPingForwarding(self.netid1, [self.netid2])

  def testEnableAnyToGroupForwarding(self):
    # enable forwarding (*, G) from if1 to if2:
    self.EnableAnyToGroupRouting(self.netid1, [self.netid2])
  ...
  def testEnableBidirectionalAnyToGroupForwarding(self):
    # enable forwarding (*, G) from if1 to if2
    self.EnableAnyToGroupRouting(self.netid1, [self.netid2])
    # enable forwarding (*, G) from if2 to if1
    self.EnableAnyToGroupRouting(self.netid2, [self.netid1])
    result = subprocess.run(['ip', '-6', 'mroute'], stdout=subprocess.PIPE)
    print(result.stdout.decode())

    self.CheckPingForwarding(self.netid1, [self.netid2])

    self.CheckPingForwarding(self.netid2, [self.netid1])

  @unittest.skipUnless(False, "skipping: waiting for kernel fix b/308390709")
  def testEnable3InterfacesAnyToGroupForwarding(self):
    # enable forwarding (*, G) from if1 to if2 and if3
    self.EnableAnyToGroupRouting(self.netid1, [self.netid2, self.netid3])
    # enable forwarding (*, G) from if2 to if1 and if3
    self.EnableAnyToGroupRouting(self.netid2, [self.netid1, self.netid3])
    # enable forwarding (*, G) from if3 to if1 and if2
    self.EnableAnyToGroupRouting(self.netid3, [self.netid1, self.netid2])
    result = subprocess.run(['ip', '-6', 'mroute'], stdout=subprocess.PIPE)
    print(result.stdout.decode())

    self.CheckPingForwarding(self.netid1, [self.netid2, self.netid3])

    self.CheckPingForwarding(self.netid2, [self.netid1, self.netid3])

    self.CheckPingForwarding(self.netid3, [self.netid1, self.netid2])
