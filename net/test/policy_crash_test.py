...
class RemovedFeatureTest(net_test.NetworkTest):
  ...
  def testPolicyNetfilterFragPanic(self):
    ...
    pkt1_payloadlen = '%04x' % (ipv6_frag_header_size + pkt1_frag_len)
    pkt2_payloadlen = '%04x' % (ipv6_frag_header_size + pkt2_frag_len)
    ...
    pkt1 = (ver6 + tclass + flowlabel + pkt1_payloadlen + nexthdr + hoplimit + src + dst
         + frag_nexthdr + frag_reserved + pkt1_frag_offset + frag_identification
         + pkt1_frag_payload)
    pkt2 = (ver6 + tclass + flowlabel + pkt2_payloadlen + nexthdr + hoplimit + src + dst
