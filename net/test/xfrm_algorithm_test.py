...
AEAD_ALGOS = [
    ...
    xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 128, 12*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 128, 12*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 128, 16*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 192, 8*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 192, 12*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 192, 16*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 256, 8*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 256, 12*8)),
    # xfrm.XfrmAlgoAead((xfrm.XFRM_AEAD_GCM_AES, 256, 16*8)),
]
...
class XfrmAlgorithmTest(xfrm_base.XfrmBaseTest):
  ...
  def InjectTests(cls):
    ...
    for aead, version, proto, name in itertools.product(
      ...
      # Produce a unique and readable name for each test. e.g.
      #     testSocketPolicySimple_cbc-aes_256_hmac-sha512_512_256_IPv6_UDP
      param_string = "%s_%d_%s_%s" % (
          aead.name, aead.key_len, 
          "IPv4" if version == 4 else "IPv6",
          "UDP" if proto == SOCK_DGRAM else "TCP")
      new_name = "%s_%s" % (func.__name__.replace("ParamTest", "test"),
                            param_string)
  ...
  def ParamTestSocketPolicySimple(self, params):
    ...
    ekey_left = os.urandom(params["crypt"].key_len / 8) if params["crypt"] is not None else None
    akey_left = os.urandom(params["auth"].key_len / 8) if params["auth"] is not None else None
    aeadkey_left = os.urandom(params["aead"].key_len / 8) if params["aead"] is not None else None
    ekey_right = os.urandom(params["crypt"].key_len / 8) if params["crypt"] is not None else None
    akey_right = os.urandom(params["auth"].key_len / 8) if params["auth"] is not None else None
    aeadkey_right = os.urandom(params["aead"].key_len / 8) if params["aead"] is not None else None
