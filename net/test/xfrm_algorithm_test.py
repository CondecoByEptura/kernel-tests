...
AUTH_ALGOS = [
    # RFC 4868 specify that the only supported truncation length is half the
    # hash size.
...
def MayAddCryptTestCase(algo_name, key_len_list, kernel_version):
  ...
  MayAddTestCase(crypt, auth, None, kernel_version, CRYPT_ALGOS,
                 crypt_algo_list)
...
# RFC 4494 specify that the only supported truncation length is half the hash size.
AUTH_AES_CMAC_KEY_TRUNC = [(128, 12 * 8)]
MayAddAuthTestCase(xfrm.XFRM_AALG_AUTH_AES_CMAC, AUTH_AES_CMAC_KEY_TRUNC,
                   (5, 4, 0))
# RFC 3566 specify that the only supported truncation length is half the hash size.
AUTH_AES_XCBC_KEY_TRUNC = [(128, 12 * 8)]
MayAddAuthTestCase(xfrm.XFRM_AALG_AUTH_AES_XCBC, AUTH_AES_XCBC_KEY_TRUNC,
                   (5, 4, 0))
...
MayAddAeadTestCase(xfrm.XFRM_AEAD_CHACHA20_POLY1305,
