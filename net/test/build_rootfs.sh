...
case "${arch}" in
  ...
  amd64)
    ...
    ;;
  armhf)
    cmdline="${cmdline} rw exitcode=/dev/ttyS0"
...
for file in common.sh stage1.sh stage2.sh; do
...
sudo chown root:root sbin/net_test.sh root/stage3.sh

# Extract the ramdisk to bootstrap with to /root
lz4 -cd "${initramfs}" | sudo cpio -D root -idum
...
kernel_version=`basename $(lz4 -cd "${raw_initrd}" | sudo cpio -idumv 2>&1 | grep usr/lib/modules/ - | head -n1)`
...
lz4 -cd "${initramfs}" | sudo cpio -idum 'lib/modules/*'
...
find * | sudo cpio -H newc -o --quiet | lz4 -9 -l -c >"${initrd}"
...
${qemu} -machine "${machine}" -cpu "${cpu}" -m 2048 >&2 \
  ...
  -append "root=/dev/vda init=/root/stage3.sh ${cmdline}"
[[ -s exitcode ]] && exitcode=`cat exitcode | tr -d '\r'` || exitcode=2
rm -f exitcode
if [ "${exitcode}" != "0" ]; then
