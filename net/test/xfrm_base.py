...
_ENCRYPTION_KEY_256 = bytes.fromhex("308146eb3bd84b044573d60f5a5fd159"
                                    "57c7d4fe567a2120f35bae0f9869ec22")
_AUTHENTICATION_KEY_128 = bytes.fromhex("af442892cdcd0ef650e9c299f9a8436a")

_ALGO_AUTH_NULL = (xfrm.XfrmAlgoAuth(("digest_null", 0, 0)), b"")
_ALGO_HMAC_SHA1 = (xfrm.XfrmAlgoAuth((xfrm.XFRM_AALG_HMAC_SHA1, 128, 96)),
                   _AUTHENTICATION_KEY_128)

_ALGO_CRYPT_NULL = (xfrm.XfrmAlgo(("ecb(cipher_null)", 0)), b"")
...
def _GetCryptParameters(crypt_alg):
  ...
  if crypt_alg == _ALGO_CRYPT_NULL:
    return (4, 0)
  elif crypt_alg == _ALGO_CBC_AES_256:
    return (16, 16)
  return (0, 0)
...
def EncryptPacketWithNull(packet, spi, seq, tun_addrs):
  ...
  padding = b"".join([i.to_bytes(1, 'little') for i in range(1, padlen + 1)])
