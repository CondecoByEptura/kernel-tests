...
def EncryptPacketWithNull(packet, spi, seq, tun_addrs):
  ...
  packet.payload = str(esp_hdr) + trailer # plen is wrong here
...
def DecryptPacketWithNull(packet):
  ...
  trailer_len = pad_len + 2 # Add size of the pad_len and next_hdr fields
  LayerType = {
  ...
  if type(packet) is scapy.IPv6:
    ...
    packet.plen -= len(xfrm.EspHdr)
  elif type(packet) is scapy.IP:
    packet.proto = IPPROTO_UDP
    packet.len -= len(xfrm.EspHdr)
