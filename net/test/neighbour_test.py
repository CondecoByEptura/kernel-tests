...
class NeighbourTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def tearDown(self):
    ...
    # restore ucast solicit to default
    iface = self.GetInterfaceName(self.netid)
    for proto in ["ipv4", "ipv6"]:
      self.SetSysctl(
          "/proc/sys/net/%s/neigh/%s/ucast_solicit" % (proto, iface),
          self.UCAST_SOLICIT_DEFAULT)
