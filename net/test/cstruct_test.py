...
class CstructTest(unittest.TestCase):
  ...
  def testNestedStructs(self):
    ...
    expected = bytes.fromhex("01" "02000000"
                             "81be" "03" "04000000"
  ...
  def testKeywordInitialization(self):
    ...
    # Populate all fields
    t1 = TestStruct(byte1=1, string2=text.encode(), int3=0xFEDCBA98, word4=0x1234)
    expected = bytes.fromhex("01" + text_bytes + "98BADCFE" "3412")
    self.assertEqual(expected, t1.Pack())

    # Partially populated
    t1 = TestStruct(string2=text.encode(), word4=0x1234)
    expected = bytes.fromhex("00" + text_bytes + "00000000" "3412")
    self.assertEqual(expected, t1.Pack())
