...
class PfKeyTest(unittest.TestCase):
  ...
  def testAddDelSa(self):
    ...
    self.pf_key.AddSa(src4.Pack(), dst4.Pack(), 0xdeadbeef, pf_key.SADB_TYPE_ESP,
                      pf_key.IPSEC_MODE_TRANSPORT, 54321,
                      pf_key.SADB_X_EALG_AESCBC, ENCRYPTION_KEY,
                      pf_key.SADB_X_AALG_SHA2_256HMAC, ENCRYPTION_KEY)

    src = csocket.Sockaddr(("2001:db8::1", 0))
    dst = csocket.Sockaddr(("2001:db8::2", 0))
    self.pf_key.AddSa(src.Pack(), dst.Pack(), 0xbeefdead, pf_key.SADB_TYPE_ESP,
    ...
    # The algorithmnames are null-terminated, but after that contain garbage.
    # Kernel bug?
    self.assertTrue(attrs4["XFRMA_ALG_CRYPT"].name.startswith("cbc(aes)\x00"))
    self.assertTrue(attrs6["XFRMA_ALG_CRYPT"].name.startswith("cbc(aes)\x00"))
    self.assertTrue(attrs4["XFRMA_ALG_AUTH"].name.startswith("hmac(sha256)\x00"))
    self.assertTrue(attrs6["XFRMA_ALG_AUTH"].name.startswith("hmac(sha256)\x00"))
