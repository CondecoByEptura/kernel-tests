...
class Xfrm(netlink.NetlinkSocket):
  ...
  def SendXfrmNlRequest(self, msg_type, req, nlattrs=None,
    ...
    for attr_type, attr_msg in nlattrs:
      # TODO: agree on how to deal with the reality that many XFRM messages use
      # nlattrs that aren't cstructs, but this function cannot support anything
      # anything that isn't a cstruct. Options include:
      #
      ...
      # 3. Require callers to pass in either a struct or a packed netlink attr,
      #    but no other types of attributes (i.e., the code below).
      # 4. Make this function somehow automatically detect what to do for
  ...
  def CreateTunnel(self, direction, selector, src, dst, spi, encryption,
    ...
    self.AddSaInfo(src, dst, spi, XFRM_MODE_TUNNEL, 0, encryption, auth_trunc,
                   None, None, sa_mark, output_mark, xfrm_if_id=xfrm_if_id)
