...
class Xfrm(netlink.NetlinkSocket):
  ...
  def AddSaInfo(self, src, dst, spi, mode, reqid, encryption, auth_trunc, aead,
    ...
    """Adds an IPsec security association for ESP.
  ...
  def MigrateTunnel(self, direction, selector, old_saddr, old_daddr,
                    ...
                    encap, new_underlying_netid, xfrm_if_id):
    """Update addresses and underlying network of Policies and an SA

    Args:
      direction: XFRM_POLICY_IN or XFRM_POLICY_OUT
      selector:An XfrmSelector of the tunnel that needs to be update.
      ...
      aead: A tuple of an XfrmAlgoAead and raw key bytes, or None.
      encap: An XfrmEncapTmpl structure, or None.
      new_underlying_netid: Net ID of the new underlying network
    ...
    # UPDSA is required to update the output_mark to point to the new underlying network.
    # All the other parameters should stay the same.
    self.AddSaInfo(new_saddr, new_daddr, spi, XFRM_MODE_TUNNEL, 0, encryption,
                   auth_trunc, aead, encap, None, new_underlying_netid, True, xfrm_if_id)
