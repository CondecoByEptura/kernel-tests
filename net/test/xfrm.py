...
class Xfrm(netlink.NetlinkSocket):
  ...
  FAMILY = NETLINK_XFRM
  _INF = 2 ** 64 -1
  _NO_LIFETIME = XfrmLifetimeCfg((_INF, _INF, _INF, _INF, 0, 0, 0, 0))
  DEBUG = False
  ...
  def MaybeDebugCommand(self, command, flags, data):
    ...
    if command == XFRM_MSG_GETSA and not flags & netlink.NLM_F_DUMP:
  ...
  def AddMinimalSaInfo(self, src, dst, spi, proto, mode, reqid,
    ...
    self.AddSaInfo(selector, xfrm_id, PaddedAddress(src), self._NO_LIFETIME,
