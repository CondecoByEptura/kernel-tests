...
class Xfrm(netlink.NetlinkSocket):
  ...
  def AddPolicyInfo(self, policy, tmpl=None):
    """Add a policy to the Security Policy Database

    Sends a new policy to the Security Policy Database
  ...
  def SendXfrmNlRequest(self,
    ...
    Send a REQUEST type netlink message

    Args:
      msg_type: an XFRM_MSG_* type
      req: an unpacked netlink request message body cstruct
      nlattrs: an unpacked list of netlink message attribute cstructs
