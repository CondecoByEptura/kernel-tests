...
class RIOTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testAcceptRaMinLftReadWrite(self):
    self.SetAcceptRaMinLft(42)
    self.assertEqual(42, self.GetAcceptRaMinLft())

  def testAcceptRaMinLftRouterLifetime(self):
    self.SetAcceptRaMinLft(100)
    ...
    time.sleep(0.01) # Give the kernel time to notice our RA
    self.assertEqual([], self.FindRoutesWithGateway())

    # RA with lifetime 50 is ignored
    self.SendRA(self.NETID, routerlft=50)
    time.sleep(0.01) # Give the kernel time to notice our RA
    self.assertEqual([], self.FindRoutesWithGateway())

    # RA with lifetime 150 is processed
    self.SendRA(self.NETID, routerlft=150)
    time.sleep(0.01) # Give the kernel time to notice our RA
  ...
  def testAcceptRaMinLftPIOLifetime(self):
    ...
    # PIO with lifetime 50 is ignored
    self.SendRA(self.NETID, piolft=50)
    time.sleep(0.01) # Give the kernel time to notice our RA
    self.assertIsNone(self.MyAddress(6, self.NETID))

    # PIO with lifetime 150 is processed
    self.SendRA(self.NETID, piolft=150)
    time.sleep(0.01) # Give the kernel time to notice our RA
  ...
  def testAcceptRaMinLftRIOLifetime(self):
    ...
    # RIO with lifetime 50 is ignored
    self.SendRIO(50, PLEN, PREFIX, PRF)
    time.sleep(0.01) # Give the kernel time to notice our RA
    self.assertFalse(self.FindRoutesWithDestination(PREFIX))

    # RIO with lifetime 150 is processed
    self.SendRIO(150, PLEN, PREFIX, PRF)
    time.sleep(0.01) # Give the kernel time to notice our RA
    self.assertTrue(self.FindRoutesWithDestination(PREFIX))
