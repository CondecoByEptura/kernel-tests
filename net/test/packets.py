...
def UDP(version, srcaddr, dstaddr, sport=0):
  ...
  if sport == 0:
    sport = _RandomPort()
  return ("UDPv%d packet" % version, ip(src=srcaddr, dst=dstaddr) / scapy.UDP(
      sport=sport, dport=53) / UDP_PAYLOAD)


def UDPWithOptions(version, srcaddr, dstaddr, sport=0):
  if version == 4:
    packet = (scapy.IP(src=srcaddr, dst=dstaddr, ttl=39, tos=0x83) / scapy.UDP(
        sport=sport, dport=53) / UDP_PAYLOAD)
  else:
    packet = (scapy.IPv6(src=srcaddr, dst=dstaddr, fl=0xbeef, hlim=39, tc=0x83)
...
def SYN(dport, version, srcaddr, dstaddr, sport=0, seq=-1):
  ...
  return ("TCP SYN", ip(src=srcaddr, dst=dstaddr) / scapy.TCP(
      sport=sport,
      dport=dport,
      seq=seq,
      ack=0,
...
def RST(version, srcaddr, dstaddr, packet):
  ...
  return ("TCP RST", ip(src=srcaddr, dst=dstaddr) / scapy.TCP(
      sport=original.dport,
      dport=original.sport,
...
def SYNACK(version, srcaddr, dstaddr, packet):
  ip = _GetIpLayer(version)
  original = packet.getlayer("TCP")
  return ("TCP SYN+ACK", ip(src=srcaddr, dst=dstaddr) / scapy.TCP(
      sport=original.dport,
      dport=original.sport,
      ack=original.seq + 1,
      seq=None,
...
def ACK(version, srcaddr, dstaddr, packet, payload=""):
  ...
  return (desc, ip(src=srcaddr, dst=dstaddr) / scapy.TCP(
      sport=original.dport,
      dport=original.sport,
      ack=original.seq + ack_delta,
      seq=original.ack,
      flags=flags,
      window=TCP_WINDOW) / payload)
...
def FIN(version, srcaddr, dstaddr, packet):
  ...
  was_syn_or_fin = (original.flags & (TCP_SYN | TCP_FIN)) != 0
  ack_delta = was_syn_or_fin + len(original.payload)
  return ("TCP FIN", ip(src=srcaddr, dst=dstaddr) / scapy.TCP(
      sport=original.dport,
      dport=original.sport,
      ack=original.seq + ack_delta,
      seq=original.ack,
  ...
  if version == 4:
    ...
    return ("ICMPv4 port unreachable",
            scapy.IP(src=srcaddr, dst=dstaddr, proto=1,
                     tos=0xc0) / scapy.ICMPerror(type=3, code=3) / packet)
  else:
    return ("ICMPv6 port unreachable",
            scapy.IPv6(src=srcaddr, dst=dstaddr) / scapy.ICMPv6DestUnreach(
                code=4) / packet)
...
def ICMPPacketTooBig(version, srcaddr, dstaddr, packet):
  if version == 4:
    return ("ICMPv4 fragmentation needed",
            scapy.IP(src=srcaddr, dst=dstaddr, proto=1) / scapy.ICMPerror(
                type=3, code=4, unused=1280) / str(packet)[:64])
  else:
...
def ICMPEcho(version, srcaddr, dstaddr):
  ...
  packet = (ip(src=srcaddr, dst=dstaddr) / icmp(id=PING_IDENT, seq=PING_SEQ) /
            PING_PAYLOAD)
...
def NS(srcaddr, tgtaddr, srcmac):
  ...
  packet = (scapy.IPv6(src=srcaddr, dst=solicited) / scapy.ICMPv6ND_NS(
      tgt=tgtaddr) / scapy.ICMPv6NDOptSrcLLAddr(lladdr=srcmac))
  return ("ICMPv6 NS", packet)


def NA(srcaddr, dstaddr, srcmac):
  packet = (scapy.IPv6(src=srcaddr, dst=dstaddr) / scapy.ICMPv6ND_NA(
      tgt=srcaddr, R=0, S=1, O=1) / scapy.ICMPv6NDOptDstLLAddr(lladdr=srcmac))
