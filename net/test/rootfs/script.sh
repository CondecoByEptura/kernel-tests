...
if [[ -n "${verbose}" ]]; then
  ...
  echo -n "$0 (pid $$, parent ${PPID}, tty $(tty)) running [$0] with args:"
  for arg in "$@"; do
    echo -n " [${arg}]"
  done
  echo
...
if [ "$script_mode" != "builder" ]; then
  # Fall out to a shell once the test completes or if there's an error.
  trap "exec /bin/bash" ERR EXIT
fi

echo -e "Running $script $script_args\n"
$script $script_args
rv="$?"

# Write exit code of script to a file so that the builder can use it
# to signal failure if anything fails.
echo "${rv}" > "${script_exitcode}"
