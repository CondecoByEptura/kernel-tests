...
# Build U-Boot image based on the Debian initrd, install grub, and create
# some additional symlinks for compatibility with the boot.cmd/boot.src
if [ -n "${embed_kernel_initrd_dtb}" ]; then
  mkimage -A arm -O linux -T ramdisk -C none -a 0 -e 0 \
    -n uInitrd -d "/boot/initrd.img-$(uname -r)" "/boot/uInitrd-$(uname -r)"

  ln -s "vmlinuz-$(uname -r)" /boot/Image
  ln -s "uInitrd-$(uname -r)" /boot/uInitrd
fi
...
load mmc ${devnum}:${distro_bootpart} 0x01f00000 /boot/dtb/rockchip/rk3399-rock-pi-4b.dtb
