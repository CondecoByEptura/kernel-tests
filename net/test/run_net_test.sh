...
cmdline="$cmdline panic=1 init=/sbin/script_init.sh"
cmdline="$cmdline script_args=\"$test_args\" script_mode=$testmode"
...
# hence script.sh needs at least 32 hex chars (which is the amount of
# hex in a single random UUID) provided to it on the kernel cmdline.
#
...
if [ "$ARCH" == "um" ]; then
  ...
  cmdline="$cmdline script=/host$SCRIPT_DIR/$test"
  ...
  cmdline="$cmdline script_exitcode=/dev/ttyS3"
  ...
  if [ "$ARCH" == "i386" -o "$ARCH" == "x86_64" -o "$ARCH" == "x86" ]; then
    ...
    cmdline="$cmdline script_exitcode=/dev/ttyS1"
  elif [ "$ARCH" == "arm64" ]; then
    # This uses a software model CPU, based on cortex-a53
    ...
    cmdline="$cmdline script_exitcode=/dev/ttyS0"
  fi
