...
fixup_ptys() {
  python  <<-'EOF'
	...
	v = fcntl.fcntl(fd, fcntl.F_GETFL)
	w = v & ~(fcntl.FASYNC | os.O_NONBLOCK | os.O_APPEND)
	fcntl.fcntl(fd, fcntl.F_SETFL, w)
EOF
...
if [ "$ARCH" == "um" ]; then
  ...
  # Pass in a 256-bit random hex seed.
  cmdline="$cmdline random_initializer=$(cat /proc/sys/kernel/random{/,/}uuid | tr -d '\n-')"
