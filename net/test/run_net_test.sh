...
if [ "$ARCH" == "um" ]; then
  ...
  # Experience shows that we need at least 128 bits of entropy for the 4.19
  ...
  #
  # However, 4.4 kernels (for which entropy is far less critical, but also
  # log warnings prior to entropy initialization) appear to require
  # significantly more: 384 bits is not sufficient.
  #
  # As such, we'll pass in 512 bits, and we'll do this as a random 88 char
  # base64 seed (because this is shorter than base16).
  # We do this by getting *four* random UUIDs and concatenating their hex
