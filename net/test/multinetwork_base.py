...
class MultiNetworkBaseTest(net_test.NetworkTest):
  ...
  @classmethod
  def SendRA(cls, netid, retranstimer=None, reachabletime=0, routerlft=-1,
             piolft=-1, m=0, o=0, options=()):
    if routerlft == -1:
      routerlft = cls.RA_VALIDITY

    if piolft == -1:
      piolft = cls.RA_VALIDITY
    ...
    if retranstimer is None:
      ...
      retranstimer = cls.RA_VALIDITY * 1000  # Lifetime is in s, retrans timer in ms.
