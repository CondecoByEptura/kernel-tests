...
class TcpFastOpenTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def CheckConnectOption(self, version):
    ...
    syn = self.ExpectPacketOn(netid, "Fastopen connect: SYN", syn)
    ...
    self.ExpectPacketOn(netid, "First connect: got SYN+ACK, expected ACK", ack)
    self.assertSocketConnected(s)
    s.close()
    desc, rst = packets.RST(version, myaddr, remoteaddr, synack)
    self.ExpectPacketOn(netid, "Closing client socket, expecting RST", rst) 
    ...
    self.ExpectPacketOn(netid, "TFO write, expected SYN with data", syn)
