...
class PingReplyThread(threading.Thread):
  ...
  def __init__(self, tun, mymac, routermac, routeraddr):
    ...
    self._tun = tun
    self._mystarted = False
    self._mystopped = False
    self._mymac = mymac
    self._routermac = routermac
    self._routeraddr = routeraddr

  def IsStarted(self):
    return self._mystarted

  def Stop(self):
    self._mystopped = True
  ...
  def SendPacket(self, packet):
    ...
    try:
      ...
      if not self._mystopped:
        raise e

  def run(self):
    self._mystarted = True
    while not self._mystopped:
      try:
...
class Ping6Test(multinetwork_base.MultiNetworkBaseTest):
  ...
  def assertValidPingResponse(self, s, data):
    ...
    self.assertEqual(data[6:].hex(), rcvd[6:].hex())
