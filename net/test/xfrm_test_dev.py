...
class XfrmFunctionalTest(xfrm_base.XfrmBaseTest):
  ...
  def _testSocketPolicy(self):
    ...
    csocket.SetSocketTimeout(s, 100)
    s.bind(("::", 53))
    saddr, sport = s.getsockname()[:2]
    saddr = "2001:db8:96::ff:fe00:9601"
    daddr = TEST_ADDR1
    ...
    # print "Normal policy"
    # xfrm_base.ApplySocketPolicy(s, AF_INET6, xfrm.XFRM_POLICY_IN,
    #                             TEST_SPI, reqid, None)
    ...
    self.CreateNewSa("::", daddr, TEST_SPI, reqid, None, True)
    self.CreateNewSa("::", saddr, TEST_SPI, reqid, None, True)
    s.sendto(net_test.UDP_PAYLOAD, (daddr, 53))
    expected_len = xfrm_base.GetEspPacketLength(xfrm.XFRM_MODE_TRANSPORT, 6,
                                                False, net_test.UDP_PAYLOAD, auth_trunc_len=xfrm_base._AUTH_NULL_TRUNC_LEN)
    ...
    incoming = (scapy.IPv6(src=daddr, dst="2001:db8:96::ff:fe00:9601") /
                 scapy.UDP(sport=sport, dport=53) /
                 net_test.UDP_PAYLOAD)
    incoming = scapy.IPv6(str(incoming))
    xfrm_base.EncryptPacketWithNull(incoming, TEST_SPI, 1)
    ...
    self.xfrm.DeleteSaInfo(daddr, TEST_SPI, IPPROTO_ESP)
    self.xfrm.DeleteSaInfo(saddr, TEST_SPI, IPPROTO_ESP)
    s.close()
  ...
  def _SetupUdpEncapSockets(self):
    ...
    encap_sock.setsockopt(IPPROTO_UDP, xfrm.UDP_ENCAP,
  ...
  def _SetupUdpEncapSaPair(self, myaddr, remoteaddr, in_spi, out_spi,
    ...
    # In the case of UDP-encap-ESP, the tests work without this policy. It
    # appears that an inbound policy is not required due to the fact that the
    # UDP_ENCAP_ESPINUDP_NON_IKE sockopt is set, and that shunts it straight
    # to the XFRM module.
    #
    # Relevant section in kernel is ipv4/udp.c:udp_lib_setsockopt()
    ...
    # print "Normal policy"
    # xfrm_base.ApplySocketPolicy(s, AF_INET, xfrm.XFRM_POLICY_IN,
    #                             in_spi, in_reqid, None)

    # print "Blocking policy"
    # xfrm_base.ApplyBlockingSocketPolicy(s, AF_INET, xfrm.XFRM_POLICY_IN,
    #                             in_spi, in_reqid, None)
  ...
  def _VerifyUdpEncapSocket(self, netid, remoteaddr, myaddr, encap_port, sock,
                           in_spi, out_spi, nullAuth=False, seq_num=1):
    # Now send a packet.
    ...
    expected_len = xfrm_base.GetEspPacketLength(xfrm.XFRM_MODE_TRANSPORT, 4,
                                                ...
                                                xfrm_base._CRYPT_NULL_IV_LEN,
                                                xfrm_base._CRYPT_NULL_BLK_SIZE,
                                                trunc_len)
    ...
    if not nullAuth and in_spi != out_spi:
      ...
      self.assertEquals(start_integrity_failures,
                        sainfo.stats.integrity_failed)
  ...
  def _RunSocketPolicyTest(self, in_spi, out_spi, use_null_auth):
    netid, myaddr, remoteaddr, encap_sock, encap_port, s = \
        self._SetupUdpEncapSockets()

    self._SetupUdpEncapSaPair(myaddr, remoteaddr, in_spi, out_spi, encap_port, s, use_null_auth)
    ...
    self._VerifyUdpEncapSocket(netid, remoteaddr, myaddr, encap_port, s, in_spi,
                              out_spi, use_null_auth)

  def testUdpEncapSameSpisNullAuth(self):
    self._RunSocketPolicyTest(TEST_SPI, TEST_SPI, True)

#   def testUdpEncapSameSpis(self):
#     self._RunSocketPolicyTest(TEST_SPI, TEST_SPI, False)

#   def testUdpEncapDifferentSpisNullAuth(self):
#     self._RunSocketPolicyTest(TEST_SPI, TEST_SPI2, True)

#   def testUdpEncapDifferentSpis(self):
#     self._RunSocketPolicyTest(TEST_SPI, TEST_SPI2, False)
...
#     self._SetupUdpEncapSaPair(myaddr, remoteaddr, start_spi, start_spi, encap_port, s, True)

#     # Check that UDP encap sockets work with socket policy and given SAs
#     self._VerifyUdpEncapSocket(netid, remoteaddr, myaddr, encap_port, s, start_spi,
#                               start_spi, True)
...
#     self._SetupUdpEncapSaPair(myaddr, remoteaddr, rekey_spi, rekey_spi, encap_port, s, True)
