...
class SockDiagTest(SockDiagBaseTest):
    ...
    for socketpair in socketpairs:
      for sock in socketpair:
        ...
        if proto == IPPROTO_UDP:
          ...
          req.id.dport, req.id.sport = req.id.sport, req.id.dport
  ...
  def testDemonstrateUdpGetSockIdBug(self):
    ...
    # e.g., sk_daddr is actually saddr, not daddr.
    # Interestingly udp_diag_destroy does not have this bug.
    # TODO: fix this upstream. It's not only clearly incorrect, but also
    # inconsistent between udp_dump_one and udp_diag_destroy.
    ...
    for version in [4, 5, 6]:
      ...
      # Create a fully-specified diag req from our socket, including cookie.
