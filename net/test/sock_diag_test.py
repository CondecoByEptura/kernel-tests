...
class SockDiagTest(SockDiagBaseTest):
  ...
  def testGetSingleUdpSocket(self):
    """Checks that getting single sockets does not require swapping src/dst."""
    for version in [4, 6]:
      ...
      for socktype in [SOCK_STREAM, SOCK_DGRAM]:
        ...
        # Make the sockets show up in the hash tables. UDP does this when we
        # bind, but for TCP we have to listen as well.
        s.bind((net_test.GetWildcardAddress(version), 0))
