...
class SockDestroyTcpTest(tcp_test.TcpBaseTest, SockDiagBaseTest):
  ...
  def testFinWait1Socket(self):
    for version in [4, 5, 6]:
      ...
      # See if we can find the resulting FIN_WAIT2 socket. At least on 3.10, we
      # can't just use the same cookie, because the cookie changes between
      # FIN_WAIT1 and FIN_WAIT2.
