...
# This test is aim for ensuring tcp keep alive offload work correctness
# when it fetch tcp information from kernel by tcp repair mode.
class TcpRepairTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  # Test the behavior of NO_QUEUE. Expect incoming data would be stored into
  # the queue, but socket can not be read/write in NO_QUEUE.
  ...
  # Test whether tcp repair socket can be poll() correctly
  # in mutiple threads at the same time.
  def testMutilThreadPoll(self):
    for version in [4, 5, 6]:
      self.PollWhenShutdownTest(version)
      self.PollWhenReceiveFinTest(version)

  def PollRepairSocketInMultiThread(self, netid, version, expected):
    sock = self.createConnectedSocket(version, netid)
    sock.setsockopt(SOL_TCP, TCP_REPAIR, TCP_REPAIR_ON)
  ...
  def PollWhenShutdownTest(self, version):
    ...
    sock, multiThreads = self.PollRepairSocketInMultiThread(netid, version, expected)
    # Test shutdown RD.
