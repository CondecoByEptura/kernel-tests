...
class TcpRepairTest(multinetwork_base.MultiNetworkBaseTest):
  ...
  def testSocketIdel(self):
    print "testSocketIdel"
    self.readQueueIdelTest(4)
    self.readQueueIdelTest(6)
    self.writeQueueIdelTest(4)
    self.writeQueueIdelTest(6)

  def readQueueIdelTest(self, version):
    netid = self.RandomNetid()
    sock = self.createConnectedSocket(version, netid)
  ...
  def writeQueueIdelTest(self, version):
    netid = self.RandomNetid()
