...
class BpfCgroupMultinetworkTest(tcp_test.TcpBaseTest):
  ...
  def ingressCgroupCountIface(self, version, packet_count, prog, test_type):
  ...
  def egressCgroupCountIface(self, version, packet_count, prog, addr, test_type):
    self.prog_fd = BpfProgLoad(BPF_PROG_TYPE_CGROUP_SKB, prog)
    BpfProgAttach(self.prog_fd, self._cg_fd, BPF_CGROUP_INET_EGRESS)
    for netid in self.NETIDS:
  ...
  def testCheckPacketifaceV4(self):
    ...
    v4addr = self.IPV4_ADDR
    packet_count = 1
    self.ingressCgroupCountIface(4, packet_count, instructionIngress, TYPE_IFACE_INGRESS)
    self.egressCgroupCountIface(4, packet_count, instructionEgress, v4addr, TYPE_IFACE_EGRESS)
