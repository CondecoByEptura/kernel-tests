...
class BpfTest(net_test.NetworkTest):
  ...
  def testPacketCount(self):
    ...
    instructions += (insBpfParamStore + BpfFuncCountPacketInit(self.map_fd)
                     + insSkFilterAccept + insPackCountUpdate
                     + insSkFilterAccept)
    self.prog_fd = BpfProgLoad(BPF_PROG_TYPE_SOCKET_FILTER, instructions)
    packet_count = 10
...
class BpfCgroupMultinetworkTest(tcp_test.TcpBaseTest):
  ...
  def TcpFullStateCheck(self, version, netid, packet_count, map_key, testType):
    ...
    for i in xrange (0, packet_count):
      ...
      # is listen_sk.
      egressCount += (1 if version == 4 else 1)
      ingressCount += 1
  ...
  def testCheckPacketProtocolV4(self):
    ...
    for netid in self.NETIDS:
      ...
      self.CheckTCPEgressTraffic(4, netid, "mark", v4addr, packet_count, IPPROTO_TCP)
      self.CheckTCPEgressTraffic(4, netid, "uid", v4addr, packet_count, IPPROTO_TCP)
      self.CheckTCPEgressTraffic(4, netid, "oif", v4addr, packet_count, IPPROTO_TCP)
      self.TcpFullStateCheck(4, netid, packet_count, IPPROTO_TCP, TYPE_PROTOCOL_EGRESS)
      # The ifindex for egress IPv6 packet is always 0.
    BpfProgDetach(self._cg_fd, BPF_CGROUP_INET_EGRESS)
