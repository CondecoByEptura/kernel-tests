...
class BpfCgroupMultinetworkTest(tcp_test.TcpBaseTest):
  ...
  def TcpFullStateCheck(self, version, netid, packet_count, map_key, test_type):
    ...
    for i in xrange (0, packet_count):
      ...
      # TODO: syn_ack packet cannot run filter since skb->sk is req_sk while sk
      # is listen_sk.
      egressCount += 1
      ingressCount += 1
    ...
    elif test_type == TYPE_IFACE_INGRESS:
      self.assertEquals(ingressCount, LookupMap(self.map_fd, map_key).value)
    elif test_type == TYPE_PROTOCOL_EGRESS:
      self.assertEquals(egressCount, LookupMap(self.map_fd, map_key).value)
    elif test_type == TYPE_PROTOCOL_INGRESS:
      self.assertEquals(ingressCount, LookupMap(self.map_fd, map_key).value)
    elif test_type == TYPE_IFACE_EGRESS:
      self.assertEquals(egressCount, LookupMap(self.map_fd, map_key).value)
    DeleteMap(self.map_fd, map_key)
  ...
  def testCheckPacketifaceV4(self):
    ...
    instructionEgress = (BpfInsn1 + INS_CGROUP_ACCEPT + BpfInsn2 + INS_BPF_PARAM_STORE +
    ...
    instructionIngress += (INS_BPF_PARAM_STORE + BpfFuncCountPacketInit(self.map_fd)
                     + INS_CGROUP_ACCEPT + INS_PACK_COUNT_UPDATE + INS_CGROUP_ACCEPT)

    v4addr = self.IPV4_ADDR
    packet_count = 1
    self.ingressCgroupCount(4, packet_count, instructionIngress, TYPE_IFACE_INGRESS)
    self.egressCgroupCount(4, packet_count, instructionEgress, v4addr, TYPE_IFACE_EGRESS)
  ...
  def testCheckPacketifaceV6(self):
    ...
    v6addr = self.IPV6_ADDR
    packet_count = 1
    self.ingressCgroupCount(6, packet_count, instructionIngress, TYPE_IFACE_INGRESS)
    self.egressCgroupCount(6, packet_count, instructionEgress, v6addr, TYPE_IFACE_EGRESS)
