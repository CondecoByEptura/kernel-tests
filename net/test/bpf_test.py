...
class BpfTest(net_test.NetworkTest):
  ...
  def testCreateMap(self):
    ...
    self.assertEquals(UpdateMap(map_fd, key, value), 0)
    ret, result = LookupMap(map_fd, key)
    self.assertEquals(ret, 0)
    self.assertEquals(result.value, 1)
    self.assertEquals(DeleteMap(map_fd, key), 0)
    ret, result = LookupMap(map_fd, key)
  ...
  def testIterateMap(self):
    ...
    ret, result = LookupMap(map_fd, 101)
    self.assertEquals(ret, -1)
    self.assertEquals(result.value, 0)
    key = 0
    count = 0
    ret, result = GetNextKey(map_fd, key)
    while ret > -1 :
      key = result.value
      self.assertGreater(key, 0)
      ret, retValue = LookupMap(map_fd, key)
      self.assertEquals(ret, 0)
      self.assertEquals(retValue.value, value)
      count = count + 1
      ret, result = GetNextKey(map_fd, key)
