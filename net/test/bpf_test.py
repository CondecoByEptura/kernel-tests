...
class BpfCgroupTest(net_test.NetworkTest):
  ...
  def expectSocketCreate(self, success):
    if success:
  ...
  def testCgroupSocketBlock(self):
    ...
    instructions = [
        ...
        BpfAlu64Imm(BPF_RSH, BPF_REG_0, 32),
        BpfJumpImm(BPF_JNE, BPF_REG_0, TEST_GID, 2),
    ]
    ...
    # The net test RunAsUid Class can only change the euid of the process
    # instead of uid. But the BPF_FUNC_get_current_uid_gid get the actual uid of
    # the current task. So it will always be 0 in net_test, we use GID to verify
    # if it can check other gid effectively.
    gid = TEST_GID
    # Socket creation with target gid should fail
    with net_test.RunAsUidGid(0, TEST_GID):
      self.expectSocketCreate(False)
    # Socket create with different gid should success
    self.expectSocketCreate(True)
    BpfProgDetach(self._cg_fd, BPF_CGROUP_INET_SOCK_CREATE)
    with net_test.RunAsUidGid(0, TEST_GID):
      self.expectSocketCreate(True)
