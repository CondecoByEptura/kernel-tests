...
# List of partitions allowed to be mounted early.
ALLOWED_EARLY_MOUNT_PARTITIONS = [
    'system',
    'odm',
    'vendor',
]
...
class DtEarlyMountTest(unittest.TestCase):
  ...
  def testOnlyAllowedPartitionsMounted(self):
    """Check that only /system, /odm, or /vendor are mounted early."""
    partitions = self.GetEarlyMountedPartitions()
    self.assertTrue(
        set(partitions).issubset(ALLOWED_EARLY_MOUNT_PARTITIONS),
        '%s are early mounted. Only %s partitions are allowed.' %
        (partitions, ALLOWED_EARLY_MOUNT_PARTITIONS))
