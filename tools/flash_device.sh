    ...
    case "$KERNEL_VERSION" in
        android-mainline)
            if [[ "$DEVICE_KERNEL_STRING" == mainline* ]] && [ ! -z "$SYSTEM_DLKM_VERSION" ]; then
                print_info "Device $SERIAL_NUMBER is with android-mainline kernel. Flash GKI directly" "$LINENO"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" == mainline* ]] && [ -z "$SYSTEM_DLKM_VERSION" ]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT without system_dlkm partition. Can't flash GKI directly." "$LINENO"
                ...
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly." "$LINENO"
        ...
        android14-6.1*)
            if [[ "$DEVICE_KERNEL_STRING" == 6.1* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ "$SYSTEM_DLKM_VERSION" == "14" ]]; then
                print_info "Device $SERIAL_NUMBER is with android14-6.1 kernel. Flash GKI directly" "$LINENO"
                flash_gki
        ...
        android14-5.15*)
            if [[ "$DEVICE_KERNEL_STRING" == 5.15* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ "$SYSTEM_DLKM_VERSION" == "14" ]]; then
                print_info "Device $SERIAL_NUMBER is with android14-5.15 kernel. Flash GKI directly" "$LINENO"
                flash_gki
