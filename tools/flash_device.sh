    ...
    case "$manifest_output" in
        ...
        *kernel/superproject*)
            if [[ "$manifest_output" == *private/google-modules/soc/gs* ]]; then
                ...
                grep -oP 'revision="\K[^"]*')
                print_info "VENDOR_KERNEL_REPO_ROOT=$VENDOR_KERNEL_REPO_ROOT"
                print_info "VENDOR_KERNEL_VERSION=$VENDOR_KERNEL_VERSION"
...
function build_platform () {
    if [[ "$SKIP_BUILD" = true ]]; then
        print_warn "--skip-build is set. Do not rebuild platform build"
        return
    fi
    ...
    if [ $exit_code -eq 1 ]; then
        print_warn "$build_cmd returned exit_code $exit_code"
        print_error "$build_cmd failed"
    else
        if [ -f "${ANDROID_PRODUCT_OUT}/system.img" ]; then
            print_info "${ANDROID_PRODUCT_OUT}/system.img exist"
        else
...
function build_slider () {
    if [[ "$SKIP_BUILD" = true ]]; then
        print_warn "--skip-build is set. Do not rebuild slider"
        return
    fi
    local build_cmd=
    ...
    print_warn "Flag --skip-build is not set. Rebuild with $build_cmd."
    if [ "$GCOV" = true ]; then
        build_cmd+=" --gcov"
    fi
    if [ "$DEBUG" = true ]; then
...
function build_ack () {
    if [[ "$SKIP_BUILD" = true ]]; then
        print_warn "--skip-build is set. Do not rebuild kernel"
        return
    fi
    build_cmd="tools/bazel run --config=fast"
    if [ "$GCOV" = true ]; then
...
function flash_platform_build() {
    if [[ "$PLATFORM_BUILD" == ab://* ]] && [ -x "$FLASH_CLI" ]; then
        local flash_cmd="$FLASH_CLI --nointeractive -w -s $SERIAL_NUMBER "
        ...
        print_info "Flash $SERIAL_NUMBER with flash station cli by: $flash_cmd"
        eval "$flash_cmd"
        exit_code=$?
        if [ $exit_code -eq 0 ]; then
    ...
    [ -x "$PLATFORM_REPO_ROOT/vendor/google/tools/flashall" ]; then
        ...
        print_info "Flash with vendor/google/tools/flashall"
    ...
    if [ ! -f "$PLATFORM_BUILD/bootloader.img" ]; then
        print_error "Cannot flash $PLATFORM_BUILD as there is no bootloader.img"
    fi
    if [ ! -f "$PLATFORM_BUILD/radio.img" ]; then
        print_error "Cannot flash $$PLATFORM_BUILD as there is no radio.img"
    fi
    local device_image=$(ls $PLATFORM_BUILD/*$PRODUCT-img*.zip)
    if [ ! -f "$device_image" ]; then
        print_error "Cannot flash $$PLATFORM_BUILD as there is no *$PRODUCT-img*.zip"
    fi
    if [ -z "$TRADEFED" ]; then
        find_tradefed_bin
    fi
    ...
    --device-image $PLATFORM_BUILD/*$PRODUCT-img*.zip --userdata-flash WIPE"
    print_info "Run $tf_cli"
    eval $tf_cli
}
...
function mixing_build() {
    ...
    if [[ "$PLATFORM_BUILD" == ab://* ]]; then
        ...
        print_info "Copy platform build $PLATFORM_BUILD to $DOWNLOAD_PATH/device_dir"
        PLATFORM_DIR="$DOWNLOAD_PATH/device_dir"
        mkdir -p "$PLATFORM_DIR"
        cd "$PLATFORM_DIR" || $(print_error "Fail to go to $PLATFORM_DIR" "$LINENO")
        local device_image=$(find "$PLATFORM_BUILD" -maxdepth 1 -type f -name *-img.zip)
        if [ ! -z "device_image" ]; then
            cp "$device_image $PLATFORM_DIR/$RPODUCT-img-0.zip"
        else
        ...
        for pattern in "${file_patterns[@]}"; do
            ...
            exit_code=$?
            if [ $exit_code -eq 0 ]; then
                print_info "Copied $PLATFORM_BUILD/$pattern to $PLATFORM_DIR"
            else
                print_error "Failed to copy $PLATFORM_BUILD/$pattern to $PLATFORM_DIR"
            fi
        done
        PLATFORM_BUILD="$PLATFORM_DIR"
    fi
    ...
    case "$KERNEL_VERSION" in
        android-mainline)
            if [[ "$DEVICE_KERNEL_STRING" == mainline* ]] && [ ! -z "$SYSTEM_DLKM_VERSION" ]; then
                ...
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" == mainline* ]] && [ -z "$SYSTEM_DLKM_VERSION" ]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT without system_dlkm partition. Can't flash GKI directly."
                print_error "Can not flash GKI to SERIAL_NUMBER without -vkb <vendor_kernel_build> been specified." "$LINENO"
            elif [[ "$DEVICE_KERNEL_STRING" != mainline* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android-mainline GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android15-6.6*)
            if [[ "$DEVICE_KERNEL_STRING" == 6.6* ]] && [ ! -z "$SYSTEM_DLKM_VERSION" ]; then
                print_info "Device $SERIAL_NUMBER is with android-mainline kernel. Flash GKI directly"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" == 6.6* ]] && [ -z "$SYSTEM_DLKM_VERSION" ]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT without system_dlkm partition. Can't flash GKI directly."
                print_error "Can not flash GKI to SERIAL_NUMBER without -vkb <vendor_kernel_build> been specified." "$LINENO"
            elif [[ "$DEVICE_KERNEL_STRING" != 6.6* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android-mainline GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android14-6.1*)
            if [[ "$DEVICE_KERNEL_STRING" == 6.1* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ "$SYSTEM_DLKM_VERSION" == "14" ]]; then
                ...
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" == 6.1* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ -z "$SYSTEM_DLKM_VERSION" ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT without system_dlkm partition. Can't flash GKI directly."
                print_error "Cannot flash android14-6.1 GKI on device $SERIAL_NUMBER." "$LINENO"
            elif [[ "$DEVICE_KERNEL_STRING" != 6.1* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android14-6.1 GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android14-5.15*)
            if [[ "$DEVICE_KERNEL_STRING" == 5.15* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ "$SYSTEM_DLKM_VERSION" == "14" ]]; then
                print_info "Device $SERIAL_NUMBER is with android14-5.15 kernel. Flash GKI directly"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" == 5.15* ]] && [[ "$DEVICE_KERNEL_STRING" == *android14* ]] && [[ -z "$SYSTEM_DLKM_VERSION" ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT without system_dlkm partition. Can't flash GKI directly."
                print_error "Cannot flash android14-5.15 GKI on device $SERIAL_NUMBER." "$LINENO"
            elif [[ "$DEVICE_KERNEL_STRING" != 5.15* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android14-5.15 GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android13-5.15*)
            if [[ "$DEVICE_KERNEL_STRING" == 5.15* ]] && [[ "$DEVICE_KERNEL_STRING" == *android13* ]]; then
                print_info "Device $SERIAL_NUMBER is with android13-5.15 kernel. Flash GKI directly"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" != 5.15* ]] || [[ "$DEVICE_KERNEL_STRING" != *android13* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android13-5.15 GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android13-5.10*)
            if [[ "$DEVICE_KERNEL_STRING" == 5.10* ]] && [[ "$DEVICE_KERNEL_STRING" == *android13* ]]; then
                print_info "Device $SERIAL_NUMBER is with android13-5.15 kernel. Flash GKI directly"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" != 5.10* ]] || [[ "$DEVICE_KERNEL_STRING" != *android13* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android13-5.10 GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        android12-5.10*)
            if [[ "$DEVICE_KERNEL_STRING" == 5.10* ]] && [[ "$DEVICE_KERNEL_STRING" == *android12* ]]; then
                print_info "Device $SERIAL_NUMBER is with android13-5.15 kernel. Flash GKI directly"
                flash_gki
            elif [[ "$DEVICE_KERNEL_STRING" != 5.10* ]] || [[ "$DEVICE_KERNEL_STRING" != *android12* ]]; then
                print_warn "Device $SERIAL_NUMBER is $PRODUCT comes with $DEVICE_KERNEL_STRING kernel. Can't flash GKI directly."
                print_error "Cannot flash android12-5.10 GKI to device directly $SERIAL_NUMBER." "$LINENO"
            fi
            ;;
        *)
    ...
    if [[ "$PLATFORM_REPO_LIST_OUT" != "error"* ]]; then
        ...
        if [ "$SKIP_BUILD" = false ]; then
            ...
            if [[ "${TARGET_PRODUCT}" == *"$PRODUCT" ]]; then
                ...
                print_error "Can not build platform build due to lunch build target failure"
            fi
        fi
...
if [[ "$KERNEL_BUILD" == ab://* ]]; then
    KERNEL_VERSION=$(echo $KERNEL_BUILD | sed "s/aosp_kernel-common-/ /g")
...
if [ -z "$PLATFORM_BUILD" ]; then  # No platform build provided
    if [ -z "$KERNEL_BUILD" ] && [ -z "$VENDOR_KERNEL_BUILD" ]; then  # No kernel or vendor kernel build
        ...
        print_error "Mixing only GKI build & vendor kernel build is not supported. Please add platform build." "$LINENO"
    ...
    if [ -z "$KERNEL_BUILD" ] && [ -z "$VENDOR_KERNEL_BUILD" ]; then  # No kernel or vendor kernel build
        ...
        if [[ "$PLATFORM_VERSION" == aosp* ]] || [[ "$PLATFORM_BUILD" == *aosp* ]]; then
            print_info "Mix vendor kernel and platform build"
            mixing_build
            flash_platform_build
        else
            print_info "Flash platform build $KERNEL_BUILD and vendor kernel $VENDOR_KERNEL_BUILD separately"
            flash_platform_build
            flash_vendor_kernel_build
        fi
