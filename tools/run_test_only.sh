...
if [[ "$TEST_DIR" == ab://* ]]; then
    ...
    cd "$REPO_ROOT_PATH/out" || exit 1

    IFS='/' read -ra array <<< "$TEST_DIR"

    # Check if array has enough elements before accessing them
    if [ ${#array[@]} -lt 6 ]; then 
        print_error "Invalid TEST_DIR format: $TEST_DIR"
        exit 1
    fi
    ...
    if [ ! -d "${array[3]}" ]; then
        mkdir -p "${array[3]}" || exit 1
    fi
    cd "${array[3]}" || exit 1

    print_info "Download remote test file ${array[5]} to $PWD with: $fetch_cmd"
    eval "$fetch_cmd"  # Consider using more secure methods if possible

    # Check if the download was successful
    if [ ! -f "${array[5]}" ]; then
        print_error "Failed to download ${array[5]}"
        exit 1
    fi
