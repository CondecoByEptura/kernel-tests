...
if [[ "$TEST_DIR" == ab://* ]]; then
    ...
    mkdir "$DOWNLOAD_PATH"
    cd $DOWNLOAD_PATH || cd $OLD_PWD || exit 1
    file_name=${TEST_DIR##*/}
    ...
    cd "$REPO_ROOT_PATH" || cd $OLD_PWD || exit 1
fi
...
if [[ "$TEST_DIR" == *".zip"* ]]; then
    ...
    if [ ! -d "$new_test_dir" ]; then
        mkdir -p "$new_test_dir" || cd $OLD_PWD || exit 1
    fi
    unzip -oq "$TEST_DIR" -d "$new_test_dir" || cd $OLD_PWD || exit 1
    case $filename in
