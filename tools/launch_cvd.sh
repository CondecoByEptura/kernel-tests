    ...
    case "$1" in
        ...
        -pb)
            ...
            if test $# -gt 0; then
                ...
                echo "platform build is not specified"
                exit 1
            fi
            shift
            ;;
        ...
        -sb)
            ...
            if test $# -gt 0; then
                ...
                echo "system build is not specified"
                exit 1
            fi
            shift
            ;;
        ...
        -kb)
            ...
            if test $# -gt 0; then
                ...
                echo "kernel build path is not specified"
                exit 1
            fi
            shift
            ;;
...
if [[ "$REPO_LIST_OUT" == *"common-modules/virtual-device"* ]] && [[ "$REPO_LIST_OUT" == *"kernel/common"* ]]; then
    if [ -z "$KERNEL_BUILD" ] && ! $SKIP_BUILD; then
        if [ ! -f "common-modules/virtual-device/BUILD.bazel" ]; then
            # TODO: Add build support to android12 and earlier kernels
            print_error "bazel build is not supported in this kernel tree. Please add --skip-build$."
            exit 1
        fi
    ...
    if [ -z "$PLATFORM_BUILD" ] && ! $SKIP_BUILD; then
        if [ -z "${TARGET_PRODUCT}"] || [[ "${TARGET_PRODUCT}" != *"cf_x86"* ]]; then
            print_error "please setup your build environment by: source build/envsetup.sh && lunch"
            exit 1
        fi
        print_warn "Flag --skip build is not set. Rebuilt platform images."
        make -j 12
